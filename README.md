...
### 서울시 결재문서(정보소통광장) 새 글 알림

`check_opengov.py`는 서울정보소통광장(opengov.seoul.go.kr) **결재문서** 중 재개발 관련 키워드(**후보지·재개발·재건축·신속통합·모아타운·정비사업**)가 든 문서를 찾아 재재보드봇(`BOARD_BOT_TOKEN`)으로 알립니다.

- opengov는 해외(GitHub Actions) 접속이 차단되고 공개 API도 없어, **네이버 웹문서 검색 오픈API**로 네이버가 색인한 opengov 결재문서(`/sanction/`)를 우회 조회합니다. 시크릿은 카페 감시와 동일한 `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET`를 공용으로 씁니다.
- **한계**: 네이버 색인에 의존하므로 실시간이 아니며(색인까지 수시간~수일 지연 가능), 색인되지 않은/비공개 문서는 빠질 수 있습니다. opengov 직접 접속·공개 API가 모두 불가능해 이것이 서버리스로 가능한 유일한 방법입니다.
- 확인한 글 링크는 `seen_opengov.json`에 저장되며, 첫 실행에서는 기존 글을 알림 없이 기준선으로 저장합니다.
