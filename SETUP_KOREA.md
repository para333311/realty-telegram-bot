...
1. **서울정보소통광장(opengov.seoul.go.kr) 결재문서** — 서울시 본청·사업소
2. **정보공개포털(open.go.kr) 기관장 결재문서** — 서울시 본청 + 14개 자치구
   (같은 10개 키워드: 재개발·재건축·신속통합·모아타운·도심복합·정비계획·정비구역·정비사업·후보지·동의서)
...
## 코드 업데이트 방법 (노트북에서)
```powershell
cd $HOME\Documents\realty-telegram-bot
git pull
```
업데이트 후 다음 30분 주기 실행 때 자동 반영됩니다. 새 감시 대상이 추가된
경우 첫 회차는 알림 없이 "감시 시작" 안내만 오고 기준선을 저장합니다.
