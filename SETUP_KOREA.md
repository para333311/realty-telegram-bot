...
- **알림 링크가 휴대폰에서 안 열릴 때**: 정보소통광장은 목록을 거치지 않고 문서
  주소로 바로 들어오는 접속을 끊어버릴 때가 있습니다(휴대폰 브라우저에
  `ERR_EMPTY_RESPONSE`). 그래서 알림을 보내기 전에 쿠키·Referer 없이 문서 주소를
  한 번 열어보고, 열리지 않으면 **제목으로 검색된 목록 링크**를 대신 보냅니다
  (목록에서 문서를 누르면 정상적으로 열립니다). 판정을 고정하고 싶으면
  `OPENGOV_LINK_MODE` 환경변수를 쓰세요 — `auto`(기본) / `search`(항상 목록 검색
  링크) / `direct`(항상 문서 주소, 예전 동작).
- 지금 어느 쪽으로 나가는지 확인: `DEBUG=1 python3 check_korea.py` 출력의
  `[DEBUG] 링크확인 ... → 열림/막힘` 줄을 보면 됩니다.
