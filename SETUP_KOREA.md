...
- **알림 링크는 결재문서 주소를 그대로 보냅니다**(`https://opengov.seoul.go.kr/
  sanction/{번호}`). 2026-07-25~28 사이 "링크가 안 열릴 때 대비" 명목으로
  사전 확인·검색 링크·네이버 링크로 여러 차례 바꿔봤지만, 원래(2026-07-17
  ~24) 잘 열리던 이 단순한 형식이 실제로는 가장 안정적이었던 것으로 확인돼
  2026-07-28에 원복했습니다. 이후 링크가 안 열리는 사례가 다시 생기면 어떤
  문서였는지(부서·제목·번호) 알려주세요.
