...
- **알림 링크가 휴대폰에서 안 열릴 때**(`ERR_EMPTY_RESPONSE`): 정보소통광장은
  목록을 거치지 않고 문서 주소로 바로 들어오는 접속을 끊어버릴 때가 있습니다.
  그래서 알림을 보내기 전에 쿠키·Referer 없이(=휴대폰에서 링크만 누른 것과 같은
  조건으로) 주소를 직접 열어보고, 열리는 주소를 골라서 보냅니다.
  1. `https://opengov.seoul.go.kr/sanction/{번호}` (짧은 형식)
  2. `https://opengov.seoul.go.kr/sanction/view/?nid={번호}` (뷰 형식)
  3. 둘 다 막히면 **제목으로 검색된 목록 링크** — 목록에서 문서를 누르면 열립니다.
- 판정을 고정하려면 `OPENGOV_LINK_MODE` 환경변수를 쓰세요 — `auto`(기본) /
  `search`(항상 목록 검색 링크) / `direct`(항상 문서 주소, 예전 동작).
- 지금 어느 쪽으로 나가는지 확인: `DEBUG=1 python3 check_korea.py` 출력의
  `[DEBUG] 링크확인 ... → 열림/막힘` 과 `알림에 넣을 링크` 줄을 보면 됩니다.
