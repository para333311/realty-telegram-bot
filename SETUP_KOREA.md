...
- **알림 링크가 항상 목록 검색 링크로 옵니다**(2026-07-27부터 기본값): 정보소통광장은
  목록을 거치지 않고 문서 주소로 바로 들어오는 접속을 끊어버릴 때가 있는데,
  "미리 열어보고 되면 직접 주소로 보낸다"는 사전확인 방식이 실제로는 신뢰할 수
  없었습니다(사전확인은 통과했는데 나중에 사용자가 누르면 `ERR_EMPTY_RESPONSE`가
  나는 사례 확인). 그래서 항상 확실히 열리는 목록 검색 링크로 고정했습니다 —
  알림의 링크를 누르면 제목으로 검색된 목록이 뜨고, 거기서 문서를 누르면 됩니다.
- `OPENGOV_LINK_MODE` 환경변수로 방식을 바꿀 수 있습니다 — `search`(기본, 항상
  목록 검색 링크) / `direct`(항상 문서 주소, 안 열릴 수 있음) / `auto`(사전확인 후
  결정 — 신뢰도 낮아 권장 안 함).
